  ...
  try {
    ...
    const eligible = snap.docs
      ...
      .filter((o) => {
        ...
        return o.subscriptionType !== "unknown" && o.leadsSentThisMonth < maxLeads;
      });

    const selected = eligible.length
      ? eligible.sort((a, b) => a.leadsSentThisMonth - b.leadsSentThisMonth)[0]
...
<p>📌 Sent by TexasMortgageLeads.com</p>
    ...
    if (selected.id) {
      await admin.firestore().collection("loanOfficers").doc(selected.id).update({
